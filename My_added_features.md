# My DeOldify modifications

## Multiple files:
After tinkering with DeOldify, I soon wanted to convert many interesting old B+W photos and videos I found on the internet. Below are instructions to convert multiple photos and
videos without intervention. Also included below is documented code for some extra features.

### Video
1. Under "Colorize!!" section of VideoColorizer.ipynb, add:
```
import os
for i in os.listdir('./video/source'):
  current = (os.path.splitext(i)[0])
```
2. Indent rest of code
3. Set file_name = current

Note: If converting many files, you may want remove ```show_image_in_notebook(result_path)``` 
<br>
<br>
<br>
...
Extra features:
```
    #Rename colorized files by adding '_Colorized.mp4'
    name = str(os.path.splitext(result_path)[0])
    os.rename(result_path, name + '_Colorized.mp4')
    
    #Try moving finished B+W footage to /video/DONE. Must create folder.
    try:
        shutil.move('./video/source/' + current, './video/DONE')
    except:
        print('Could not find B+W video for move.')
        
    #Try removing _no_audio.mp4 file
    try:
        os.remove(name + '_no_audio.mp4')
    except:
        print('_no_audio not present')
        
    #Try removing unneeded audio file
    try:
        os.remove('./video/source/' + current + '.aac')
    except:
        print('Audio file not present')
```
### Images:
1. Under "Colorize!!" section of ImageColorizer.ipynb
```
import glob
for i in glob.glob("[Your photos folder]/**/**.jpg", recursive=True):
```
2. Indent code under loop

Note: If converting many files, you may want remove ```show_image_in_notebook(result_path)``` 
<br>
<br>

## Reuse colorized frames
After using DeOldify a bit, I noticed every time the code was run, previously generated B+W and colorized frames were erased. While converting an especially long B+W video, 
two power outages required twice redoing over 8 hours of runtime. I decided to modify the code to pick up colorizing frames wherever the code last stopped. Also the code below
prevents regenerating B+W frames from video.

Note: If you want to regenerate frames for a different render_factor, delete /video/colorframes/[Name of video].
<br>

Copy of modified /deoldify/visualize.py file:
...
class VideoColorizer:
    ...
    def _extract_raw_frames(self, source_path: Path):
        ...
        if not path.exists(bwframes_folder):
            bwframes_folder.mkdir(parents=True, exist_ok=True)
            ffmpeg.input(str(source_path)).output(
                str(bwframe_path_template), format='image2', vcodec='mjpeg', qscale=0
            ).run(capture_stdout=True)
        else:
            print('Found existing B+W frames for' + str(source_path) + '. To regenerate frames, delete ' + str(bwframes_folder))
    ...
    def _colorize_raw_frames(
        ...
        if path.exists(colorframes_folder):
            print('Found existing colorized frames for' + str(source_path) + '. To regenerate frames, delete ' + str(colorframes_folder))
            bw_arry = os.listdir(bwframes_folder)
            color_arry = os.listdir(colorframes_folder)
            i = 0
            for i in color_arry:
                try:
                    bw_arry.remove(i)
        ...
        for img in progress_bar(bw_arry):
    ...
    def _build_video(self, source_path: Path) -> Path:
        ...
        print('Video created here: ' + str(result_path).replace('.mp4', '_Colorized.mp4'))
        return result_path
